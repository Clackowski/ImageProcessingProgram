...
/**
 * The test class for all IImageOperations. Tests apply() method for every non-abstract operation
 * class.
 */
public class ImageOperationsTest {
  ...
  List<List<IPixel>> expectedMosaic4;
  ...
  public void init() {
    ...
    this.testImageMosaic4 = ImageUtil.readImage("res" + File.separator +
        "test-image-6-by-5-rainbow-brightness-gradient-sepiad.ppm");
    ...
    this.mosaic4 = new MosaicOperation(4);
    ...
    this.expectedMosaic4 = this.imageToPixels(this.testImageMosaic4);
  }
  ...
  @Test
  public void testMosaic4() {
    Map<List<Integer>, List<List<Integer>>> m = this.mosaic4.generateSeeds(this.testImageMosaic4,
        new Random(5), new Random(4));

    int[] s = {379066948, -1667228448, 758500184, -1157408321};
    int[] s2 = {-349120689, -812936179, -396984392, -1157023572};

    int[] x = new int[m.size()];
    int[] y = new int[m.size()];

    int i = 0;
    for (Map.Entry<List<Integer>, List<List<Integer>>> entry : m.entrySet()) {
      x[i] = entry.getKey().get(0);
      y[i] = entry.getKey().get(1);
      i++;
    }

    assertArrayEquals(s, x);
    assertArrayEquals(s2, y);
  }
